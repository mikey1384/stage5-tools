...
import { calvoWildProjectCopy } from "./copy";

const post = posts.find((p) => p.slug === "calvo-wild-project")!;

export async function generateMetadata(): Promise<Metadata> {
  const locale = await getLocale();
  const copy = calvoWildProjectCopy[locale as keyof typeof calvoWildProjectCopy];
  if (!copy) {
    notFound();
  }
...
export default async function CalvoWildProjectPage({
  ...
  const structuredData = {
    ...
    headline: copy.h1,
    description: copy.description,
    url: `https://translator.tools${localizeHref("/watch/calvo-wild-project")}`,
    datePublished: "2026-08-21",
    author: { "@type": "Organization", name: "Stage5 Tools" },
    publisher: { "@type": "Organization", name: "Stage5 Tools", url: "https://translator.tools" },
    about: [
      { "@type": "Person", name: "Alejandro G. Calvo", description: "Spanish film critic" },
      {
        "@type": "VideoObject",

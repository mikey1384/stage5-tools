...
export function YouTubeDemo({
    ...
    const loadCaptions = async () => {
      try {
        const response = await fetch(
          `/watch/kore-eda-piarchia.${selectedLang}.30s.vtt`
        );
        if (response.ok) {
  ...
  const initPlayer = () => {
    ...
    playerRef.current = new window.YT.Player("youtube-player-koreeda", {
      videoId: "j29oHrGMmtY",
      playerVars: {
        rel: 0,
        modestbranding: 1,
  ...
  return (
    <div ref={containerRef} className="relative">
      <div className="aspect-video overflow-hidden rounded-2xl border border-white/10 bg-black">
        <div id="youtube-player-koreeda" className="h-full w-full" />

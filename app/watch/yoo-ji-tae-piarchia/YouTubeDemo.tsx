...
export function YouTubeDemo({
    ...
    const loadCaptions = async () => {
      try {
        const response = await fetch(
          `/watch/yoo-ji-tae-piarchia.${selectedLang}.30s.vtt`
        );
        if (response.ok) {
  ...
  const initPlayer = () => {
    ...
    playerRef.current = new window.YT.Player("youtube-player-yoo", {
      videoId: "PYY10Yq50bA",
      playerVars: {
        rel: 0,
        modestbranding: 1,
  ...
  return (
    <div ref={containerRef} className="relative">
      <div className="aspect-video overflow-hidden rounded-2xl border border-white/10 bg-black">
        <div id="youtube-player-yoo" className="h-full w-full" />
